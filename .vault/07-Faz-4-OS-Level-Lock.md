...
- [x] WebSocket'ten "write_start" ve "write_end" sinyalleri alınıyor
- [x] `fs.chmodSync()` ile dosya izinleri değiştirilebiliyor
- [x] Dosya salt okunur hale gelmiş yazma denemesi `EACCES` hatası veriyor
- [x] Debounce mekanizması çalışıyor (varsayılan 500ms)
- [x] Eklenti kapatılırken (`deactivate`) tüm kilitli dosyalar açılıyor
- [x] Güvenlik ağı (cleanup) çalışıyor
- [x] WebSocket message format doğru şekilde parse ediliyor
- [x] İki editor arasında yazma çakışması yok
